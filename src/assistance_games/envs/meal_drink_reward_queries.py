...
class MealDrinkGridPerfectQuery(AssistanceGame):
    ...
    def __init__(self, spec):
        ...
        # this one works! (asks about r0, if it's 3 does that, otherwise chooses the other option).
        ...
        # and the 3 ways to respond to each query response (separate queries don't get separate response actions)
    ...
    def make_transition_and_reward_matrices(self):
        T_dict = {}
        reward_dicts = {x: {} for x in self.meal_reward_possibilities}
        ...
        for idx in range(self.nS):
            ...
            for a_r in range(self.robot_action_space.n):
                ...
                for a_h in range(self.human_action_space.n):
                    ...
                    # rewards
                    for r_cake, r_pizza in self.meal_reward_possibilities:
                        reward_dicts[(r_cake, r_pizza)][idx, a_h, a_r, next_idx] = self.reward_fn(s, r_cake, r_pizza)
    ...
    def reward_fn(self, s, r_pizza, r_cake):
        r = 0
        if s.meal_timer == 0:
            if s.meal == 2:
                r += r_pizza
...
def human_response_meal_reward_query(assistance_game, reward, **kwargs):
    ag = assistance_game
    reward_idx = kwargs['reward_idx']
    n_r_options_per_query = 2
    response_action_idx = list(product(range(n_r_options_per_query), range(n_r_options_per_query)))[reward_idx]
    ...
    for idx in range(ag.nS):
        ...
        # meal option 1 query
            ...
            policy[idx, 6 + response_action_idx[0]] = 1.0
        # meal option 2 query
        elif s.query == 2:
            policy[idx, 6 + response_action_idx[1]] = 1.0
    return policy


class MealDrinkGridPerfectQueryProblem(AssistanceProblem):
    ...
    def __init__(self, use_belief_space=True):
        spec = self.Spec(height=3,
                         ...
                         init_h_away_timer=0,
                         max_h_away_timer=0)
        human_policy_fn = human_response_meal_reward_query
        self.assistance_game = MealDrinkGridPerfectQuery(spec)
